/* Иконографика: тонкие линейные SVG-иконы (stroke, 24×24).
   Категории + интерфейсные. Экспортируются в window.Icon / window.CatIcon. */
(function () {
  const React = window.React;
  const h = React.createElement;

  function Svg(props, children) {
    const { size = 24, stroke = 1.6, fill = "none", ...rest } = props || {};
    return h(
      "svg",
      Object.assign(
        {
  ...
  const PATHS = {
    ...
    spark: ["path:M12 3v4", "path:M12 17v4", "path:M3 12h4", "path:M17 12h4", "path:M6 6l2.5 2.5", "path:M15.5 15.5L18 18", "path:M18 6l-2.5 2.5", "path:M8.5 15.5L6 18"],
    leaf: ["path:M5 20c0-8 6-13 14-13 0 8-5 14-13 14", "path:M5 20c3-4 6-6 9-8"],
    pulse: ["path:M3 12h4l2-5 4 12 2.5-7H21"],
    menu: ["path:M4 7h16", "path:M4 12h16", "path:M4 17h16"],
    quote: ["path:M9 8.5C7 9 6 10.5 6 13v3h4v-4H8c0-1.6.5-2.5 1.6-3zM17 8.5c-2 .5-3 2-3 4.5v3h4v-4h-2c0-1.6.5-2.5 1.6-3z"],
  };

  function Icon(props) {
    const { name, ...rest } = props;
    const def = PATHS[name] || PATHS.box;
    const kids = def.map((d, i) => {
      const [type, args] = d.split(":");
      const n = args.split(" ").map(Number);
      if (type === "circle") return h("circle", { key: i, cx: n[0], cy: n[1], r: n[2] });
      if (type === "rect")
        return h("rect", { key: i, x: n[0], y: n[1], width: n[2], height: n[3], rx: n[4] || 0 });
      return h("path", { key: i, d: args });
    });
    return Svg(rest, kids);
  }
  ...
  function CatIcon(props) {
    ...
    const map = {
      ...
      // для клиник — корпус/крест
      clinic: ["path:M4 21V6l8-3 8 3v15", "path:M4 21h16", "path:M12 8.5v4", "path:M10 10.5h4", "path:M9 21v-3h6v3"],
    };
    const def = map[name] || map.clinic;
    return Svg(Object.assign({ stroke: 1.5 }, rest), def.map((d, i) => {
      const [type, args] = d.split(":");
      const n = args.split(" ").map(Number);
      if (type === "circle") return h("circle", { key: i, cx: n[0], cy: n[1], r: n[2] });
      if (type === "rect") return h("rect", { key: i, x: n[0], y: n[1], width: n[2], height: n[3], rx: n[4] || 0 });
      return h("path", { key: i, d: args });
    }));
  }

  Object.assign(window, { Icon, CatIcon });
})();
